...
```execute-1 
cd partnertapdemo && git init && git add . && git commit -m "updated changes" && git remote add origin https://$GIT_HOST/tapdemo-user/partnertapdemo  && git push https://$GIT_USERNAME:$GIT_PASSWORD@$GIT_HOST/tapdemo-user/$SESSION_NAME HEAD:master --force && cd ~
```
